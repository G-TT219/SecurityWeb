package org.example.servlet;

import java.util.Scanner;

public class Sm4 {
	public  Sm4() {
		
	}
	//https://www.cnblogs.com/kentle/p/14135865.html
	//��Կ��01 23 45 67 89 ab cd ef fe dc ba 98 76 54 32 10
	//1234567812345678
	private final static int[] CK={
        0x00070e15, 0x1c232a31, 0x383f464d, 0x545b6269,
        0x70777e85, 0x8c939aa1, 0xa8afb6bd, 0xc4cbd2d9,
        0xe0e7eef5, 0xfc030a11, 0x181f262d, 0x343b4249,
        0x50575e65, 0x6c737a81, 0x888f969d, 0xa4abb2b9,
        0xc0c7ced5, 0xdce3eaf1, 0xf8ff060d, 0x141b2229,
        0x30373e45, 0x4c535a61, 0x686f767d, 0x848b9299,
        0xa0a7aeb5, 0xbcc3cad1, 0xd8dfe6ed, 0xf4fb0209,
        0x10171e25, 0x2c333a41, 0x484f565d, 0x646b7279
};
	private final static int[] FK= { 0xa3b1bac6, 0x56aa3350, 0x677d9197, 0xb27022dc};

	
	public static void main(String[] args) {
		// TODO Auto-generated method stub
		Scanner scanner=new Scanner(System.in);
		System.out.println("���ģ�");
//		byte[] input=Hex2Byte(scanner.nextLine());
		byte[] input=scanner.nextLine().getBytes();
		System.out.println("���ģ�");
		byte[] key=Hex2Byte(scanner.nextLine());
//		byte[] input=scanner.nextLine().getBytes();
//		byte[] key=scanner.nextLine().getBytes();
		byte[] message=SM4(input,key,0);;
		int time=scanner.nextInt();
		while(time-->1) {
			message=SM4(message,key,0);
		}
		System.out.println( bytesToHexString(message)+"\n"+bytesToHexString(SM4(message, key, 1)));
	}
	
	//��input����nλ
	private static int left_move(int input ,int n) {
		return (input>>>(32-n))|(input <<n); //
	}
	
	//32λ�ֳ�4��8λ
	private static byte[] spliteTo8bit(int n) {
		return	new byte[]{(byte)(n>>>24),(byte)(n>>>16),(byte)(n>>>8),(byte)n};

	}
	private static int combinTo32bit(byte b0,byte b1,byte b2,byte b3) {
		return ((b0 & 0xFF)<<24)|((b1 & 0xFF)<<16)|((b2 & 0xFF)<<8)|(b3 & 0xFF);
	}
	
	//�б任
	private static int SboxTransform(int input) {
		 //s�еĲ���
        final int[] SBOX = {
                
        };

        byte[] temp = spliteTo8bit(input);//���32������
        byte[] output = new byte[4];//�����б任���
        //�б任
        for (int i = 0; i < 4; i++) {
            output[i] = (byte) SBOX[temp[i] & 0xFF];
        }
        //��4��8λ�ֽںϲ�Ϊһ������Ϊ�б任���
        return combinTo32bit(output[0], output[1], output[2], output[3]);
	}
	public static byte[] SM4(byte[] messge,byte[] key,int mode) {
		int[] rk=getKey(key);
		int[] text=new int[4];
		for(int i=0;i<4;i++) {
			text[i]=combinTo32bit(messge[4*i], messge[4*i+1], messge[4*i+2], messge[4*i+3]);
		}
		for (int i = 0; i < 32; i++) {
			int index=(mode==0)?i:31-i;
			int sbox_input=text[1]^text[2]^text[3]^rk[index];
			int sbox_output=SboxTransform(sbox_input);
			int xfinal=sbox_output^left_move(sbox_output, 2)^left_move(sbox_output, 10)^left_move(sbox_output, 18)^
					left_move(sbox_output, 24)^text[0];
			text[0]=text[1];
			text[1]=text[2];
			text[2]=text[3];
			text[3]=xfinal;
		}
		byte[] output=new byte[16];
		for (int i = 0; i < 4; i++) {
            System.arraycopy(spliteTo8bit(text[3 - i]), 0, output, 4 * i, 4);
        }
		return output;
	}
	private static int[] getKey(byte[] input) {
		int[] key=new int[32];
		int[] text=new int[4];
		for(int i=0;i<4;i++) {
			text[i]=combinTo32bit(input[4*i], input[4*i+1], input[4*i+2], input[4*i+3]);
		}
		int[] k=new int[4];
		for(int i=0;i<4;i++) {
			k[i]=text[i]^FK[i];
		}
		for(int i=0;i<32;i++) {
			int sbox_input=k[1]^k[2]^k[3]^CK[i];
			int sbox_output=SboxTransform(sbox_input);
			key[i]=sbox_output^left_move(sbox_output, 13)^left_move(sbox_output, 23)^k[0];
			k[0]=k[1];
			k[1]=k[2];
			k[2]=k[3];
			k[3]=key[i];
		}
		return key;
	}
	 public static String bytesToHexString(byte[] bytes) {
	        StringBuilder builder = new StringBuilder();
	        for (byte b : bytes) {
	            builder.append(String.format("%s", b));
	            builder.append(" ");
	        }
	        return builder.toString();
	    }
	    //16�����ַ���תbyte����
		public static byte[] Hex2Byte(String inHex) {
			
			String[] hex=inHex.split(" ");//�����յ��ַ������ո�ָ������
			byte[] byteArray=new byte[hex.length];
			
			for(int i=0;i<hex.length;i++) {
				//parseInt()�������ڽ��ַ���������Ϊ�з��ŵ�n�����������н���
				byteArray[i]=(byte)Integer.parseInt(hex[i],16);
			}
			
			return byteArray;
			
		}
		
		private static int[] byteToASC2Code(byte[] bytes) {
			int[] asc=new int[bytes.length];
			for(int i=0;i<bytes.length;i++) {
				asc[i]=(int)bytes[i];
			}
			return asc;
		}
		
		
		
		
		
		
		
}